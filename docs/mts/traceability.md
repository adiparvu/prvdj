...
| A project survives being closed | MP#9, ADR-0003 | `prv-ffi::Engine::document` | `a_project_saved_and_opened_is_the_same_project`, `a project written to disk comes back the same project` | **Verified** |
| A saved project and a synchronised one are one format | MP#24, ADR-0003 | `prv-project::wire::encode_all` | `a_project_file_carries_what_this_build_cannot_read` | **Verified** |
| Reopening never re-uses an identity the file spent | MP#9, ADR-0003 | `prv-ffi::Engine::absorb_placement_identities` | `editing_after_opening_does_not_reuse_a_number_the_file_already_used`, `adding a track after opening does not overwrite one that was saved` | **Verified** |
| Opening what is not there is an answer, not a failure | MP#13 | `PRVKit::Session::open(from:named:)` | `opening something that is not there is an answer, not a failure` | **Verified** |
